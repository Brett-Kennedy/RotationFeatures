...
def test_classification_default_parameters_max_four(datasets_tester, partial_result_folder, results_folder):
	if NUM_DATASETS_CLASSIFICATION_DEFAULT <= 0:
		return

	print_header("Classification with max depth of 4")

	pipe1 = Pipeline([('dt', tree.DecisionTreeClassifier(max_depth=4, random_state=0))])
	pipe2 = Pipeline([('rota', RotationFeatures()), ('dt', tree.DecisionTreeClassifier(max_depth=4, random_state=0))])
...
def test_classification_grid_search(datasets_tester, partial_result_folder, results_folder):
	...
	if NUM_DATASETS_CLASSIFICATION_GRID_SEARCH <= 0:
		return
	...
	rota_parameters = {
		'dt__max_depth': (3, 4, 5, 6, 100),
		'rota__degree_increment': (3, 4, 10, 15, 20, 30)
	}
...
def test_regression_default_parameters_max_four(datasets_tester, partial_result_folder, results_folder):            
	if NUM_DATASETS_REGRESSION_DEFAULT <= 0:
		return 

	print_header("Regression with max depth of 4")

	pipe1 = Pipeline([('dt', tree.DecisionTreeRegressor(max_depth=4, random_state=0))])
	pipe2 = Pipeline([('rota', RotationFeatures()), ('dt', tree.DecisionTreeRegressor(max_depth=4, random_state=0))])
...
def test_regression_grid_search(datasets_tester, partial_result_folder, results_folder):
	...
	if NUM_DATASETS_REGRESSION_GRID_SEARCH <= 0:
		return 
